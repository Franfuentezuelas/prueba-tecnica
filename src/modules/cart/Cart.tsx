...
export default function Cart({ reload, pasoActual, setPasoActual, mainState }: CartProps) {
  ...
  const calcularTotal = (): number => {
    return printCartProducts().reduce((total, cartProduct) => total + cartProduct.qty * cartProduct.price, 0)
  }
  ...
  return (
    <aside className={styles.aside}>
        ...
        {printCartProducts().map(cartProduct => (
          <div key={cartProduct.id} className={styles.cartItem}>
            ...
            <div className={styles.acciones}>
              <p className={styles.total}>{(cartProduct.qty * cartProduct.price).toFixed(2)}€</p>
              <button
      ...
      <h2 className={styles.totalCarrito}>Total: {calcularTotal().toFixed(2)}€</h2>
      <div className={styles.botones}>
        <button className={styles.botonAnterior} disabled={pasoActual === 'inicio'} onClick={() => handleContinuar(-1)}>
          Anterior
        </button>
        <button
